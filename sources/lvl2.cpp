...
template<typename T> std::string number_to_string(const T& n) { //Pouvoir faire appara�tre la variable lives en jeu � c�t� de "Lives = "
 ...
 void lvl2(sf:: Text &text,sf::RenderWindow &window, sf::Texture &stop, sf::Sprite &sprite,sf::CircleShape &circle,sf::RectangleShape &rectangle,sf::RectangleShape &Briques, sf::RectangleShape &bord, bool &pause, int** brique, bool &perd,int &niveau){
    ...
    sf::Texture Tbackground2; //On cr�e une texture "Tbackground"
    sf::Sprite Sbackground2; //On cr�e un Sprite "background"
    ...
    Sbackground2.scale(sf::Vector2f(0.43f,0.58f)); // on change sa taille pour l'adapter � la fenetre
    Sbackground2.setOrigin(sf::Vector2f(10,10)); // on la d�place l�g�rement sur la gauche
    ...
    sf::Event event;
        ...
        if (pause){ // Si la variable pause est true
            ...
            if(perd){
                perdu(event,Sbackground2,window,rectangle,bord,circle,Briques,brique,lives,x,y,vitX,vitY,perd,rectX,MouseX,niveau);
                text.setString("Lives = " + number_to_string(lives));
            }
            ...
            disparitonRebondSurBrique(brique, x, y, vitX, vitY, 4, 4,1,1);
            disparitonRebondSurBrique(brique, x, y, vitX, vitY, 4, 9,1,6);
            disparitonRebondSurBrique(brique, x, y, vitX, vitY, 4, 14,1,11);
        ...
        creationBrique(window, Briques, brique, 4, 4,1,1);
        creationBrique(window, Briques, brique, 4, 9,1,6);
        creationBrique(window, Briques, brique, 4, 14,1,11);
